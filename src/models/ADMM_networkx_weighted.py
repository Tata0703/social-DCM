...
class ADMM_networkx_weighted:
	...
	def update_b(self):
		...
		for i in range(self.num_nodes):
			bi = cvx.Variable(1)
			# i = NI.GetId()
			loss = cvx.logistic(-cvx.mul_elemwise(self.y[i], self.X[i].dot(self.W)+bi))*self.temp.node[i]['pos_node_prob']
	...
	def runADMM_Grid(self,iterations):
		for i in range(iterations):
			...
			self.update_U()
			if i%1 == 0:
				print 'iteration = ',i, 'objective = ', self.cal_LL()[0]
